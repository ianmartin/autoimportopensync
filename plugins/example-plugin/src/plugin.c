...
static void connect(OSyncContext *ctx)
{
	...
	osync_hashtable_load(env->hashtable, env->member);
	
	//you can also use the anchor system to detect a device reset
	//or a path change here (optional)
...
static void get_changeinfo(OSyncContext *ctx)
{
	 ...
	 */
		...
		osync_change_set_data(change, <pointer to data>, sizeof(data), TRUE);			
...
void get_info(OSyncPluginInfo *info)
{
	...
	info->description = "A longer description. < 200 chars";
	...
	info->functions.get_data = get_data;
