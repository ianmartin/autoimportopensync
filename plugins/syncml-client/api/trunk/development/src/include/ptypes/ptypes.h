...
ptpublic extern wchar_t* emptystr;
...
protected:
    ...
    void initialize(const wchar_t*, int);
    void initialize(const wchar_t*);
    void initialize(wchar_t);
    void initialize(const string& s);
    void initialize(const wchar_t*, int, const wchar_t*, int);
////void initialize(const variant& v);
    void finalize();

    void assign(const wchar_t*, int);
    void assign(const wchar_t*);
    void assign(const string&);
    void assign(wchar_t);

    string(const wchar_t* s1, int len1, const wchar_t* s2, int len2)  { initialize(s1, len1, s2, len2); }

public:
    ptpublic friend int  length(const string& s);
    ptpublic friend int  refcount(const string& s);
    ptpublic friend void assign(string& s, const wchar_t* buf, int len);
    ptpublic friend void clear(string& s);
    ptpublic friend bool isempty(const string& s);

    ptpublic friend void   setlength(string&, int);
    ptpublic friend wchar_t*  unique(string&);
    ptpublic friend void   concat(string& s, const wchar_t* sc, int catlen);
    ptpublic friend void   concat(string& s, const wchar_t* s1);
    ptpublic friend void   concat(string& s, wchar_t s1);
    ptpublic friend void   concat(string& s, const string& s1);
    ptpublic friend string copy(const string& s, int from, int cnt);
    ptpublic friend void   ins(const wchar_t* s1, int s1len, string& s, int at);
    ptpublic friend void   ins(const wchar_t* s1, string& s, int at);
    ptpublic friend void   ins(wchar_t s1, string& s, int at);
    ptpublic friend void   ins(const string& s1, string& s, int at);
    ptpublic friend void   del(string& s, int at, int cnt);
    ptpublic friend int    pos(const wchar_t* s1, const string& s);
    ptpublic friend int    pos(wchar_t s1, const string& s);
    ptpublic friend int    rpos(wchar_t s1, const string& s);
    ptpublic friend bool   contains(const wchar_t* s1, int len, const string& s, int at);
    ptpublic friend bool   contains(const wchar_t* s1, const string& s, int at);
    ptpublic friend bool   contains(wchar_t s1, const string& s, int at);
    ...
    string()                                      { initialize(); }
    string(const wchar_t* sc, int initlen)          { initialize(sc, initlen); }
    string(const wchar_t* sc)                       { initialize(sc); }
    string(wchar_t c)                                { initialize(c); }
    ...
    string& operator=  (const wchar_t* sc)          { assign(sc); return *this; }
    string& operator=  (wchar_t c)                  { assign(c); return *this; }
    string& operator=  (const string& s)          { assign(s); return *this; }
    string& operator+= (const wchar_t* sc)          { concat(*this, sc); return *this; }
    string& operator+= (wchar_t c)                  { concat(*this, c); return *this; }
    string& operator+= (const string& s)          { concat(*this, s); return *this; }

    string  operator+  (const wchar_t* sc) const;
    string  operator+  (wchar_t c) const;
    string  operator+  (const string& s) const;

    ptpublic friend string operator+ (const wchar_t* sc, const string& s);
    ptpublic friend string operator+ (wchar_t c, const string& s);

    bool    operator== (const wchar_t* sc) const    { return wcscmp(data, sc) == 0; }
    bool    operator== (wchar_t) const;
    bool    operator== (const string&) const;
    bool    operator!= (const wchar_t* sc) const    { return !(*this == sc); }
    ...
    ptpublic friend void initialize(string& s, const wchar_t* s1);
...
inline void assign(string& s, const wchar_t* buf, int len) { s.assign(buf, len); }
...
inline void initialize(string& s, const wchar_t* s1)       { s.initialize(s1); }
inline void finalize(string& s)                          { s.finalize(); }

#ifndef CHECK_BOUNDS
inline wchar_t& string::operator[] (int i)                 { return unique(*this)[i]; }
#endif
...
ptpublic string fill(int width, wchar_t pad);
ptpublic string pad(const string& s, int width, wchar_t c, bool left = true);

ptpublic string itostring(large value, int base, int width = 0, wchar_t pad = 0);
ptpublic string itostring(ularge value, int base, int width = 0, wchar_t pad = 0);
ptpublic string itostring(int value, int base, int width = 0, wchar_t pad = 0);
ptpublic string itostring(unsigned value, int base, int width = 0, wchar_t pad = 0);
...
ptpublic large  stringtoi(const wchar_t*);
ptpublic large  stringtoie(const wchar_t*);
ptpublic ularge stringtoue(const wchar_t*, int base);

ptpublic string lowercase(const wchar_t* s);
ptpublic string lowercase(const string& s);

// itobase is now alias to itostring, deprecated
ptpublic string itobase(large value, int base, int width = 0, wchar_t pad = 0);


wchar_t hex4(char c);

inline wchar_t locase(wchar_t c)
    { return towlower(c); }

inline wchar_t upcase(wchar_t c)
    { return towupper(c); }

inline int hstrlen(const wchar_t* p) // some Unix systems do not accept NULL
    { return p == nil ? 0 : wcslen(p); }
...
//    void initialize(const wchar_t* v)       { tag = VAR_STRING; PTYPES_NAMESPACE::initialize(PTR_TO_STRING(value.s), v); }
...
//    variant(const wchar_t* v)               { initialize(v); }
    ...
    const variant& operator[](const wchar_t* key) const   { return get(*this, string(key)); }
...
//inline void initialize(variant& v, const wchar_t* i) { v.initialize(i); }
