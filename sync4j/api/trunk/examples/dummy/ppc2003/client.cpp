...
void displayItems(SyncSource& s) {
    ...
    for (i=0; i<n; ++i) {
        wsprintf(logmsg, TEXT("client side - key: %s, data: %s"), items[i]->getKey(NULL), items[i]->getData());
        LOG.info(logmsg);
    }
    ...
    LOG.debug(TEXT("client side - ============="));
    for (i=0; i<n; ++i) {
        wsprintf(logmsg, TEXT("client side - key: %s"), items[i]->getKey(NULL), items[i]->getData());
        LOG.info(logmsg);
    }
...
void displayMappings(SyncSource& s) {
    ...
    for (unsigned int i=0; i<n; ++i) {
        wsprintf(logmsg, TEXT("client side - luid: %s, guid: %s"), map[i]->getLUID(NULL), map[i]->getGUID(NULL));
        LOG.info(logmsg);
    }
...
void displayItemStatus(SyncSource& s) {
    ...
    LOG.debug(TEXT("client side - ========"));
    for (unsigned int i=0; i<n; ++i) {
        wsprintf(logmsg, TEXT("client side - key: %s, data: %i, cmd: %s, cmdRef: %i, msgRef: %i, cmdID:%i"), 
