...
static OSyncConvCmpResult compare_vjournal(const char *leftdata, unsigned int leftsize, const char *rightdata, unsigned int rightsize)
{
...
static osync_bool detect_plain_as_vjournal(const char *data, int size)
{
...
static void create_vjournal(char **data, unsigned int *size)
{
	*data = g_strdup_printf("BEGIN:VJOURNAL\r\nDESCRIPTION:%s\r\nSUMMARY:%s\r\nEND:VJOURNAL", osync_rand_str(20), osync_rand_str(6));
	*size = strlen(*data);	
}

static void destroy_vjournal(char *input, unsigned int inpsize)
{
	g_free(input);
}
...
osync_bool get_format_info(OSyncFormatEnv *env, OSyncError **error)
{
	...
	osync_objformat_set_create_func(format, create_vjournal);
