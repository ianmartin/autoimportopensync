...
void gpe_contacts_commit_change (void *userdata, OSyncPluginInfo *info, OSyncContext *ctx, OSyncChange *change)
{
	...
	switch (osync_change_get_changetype (change)) {
		case OSYNC_CHANGE_TYPE_DELETED:
			osync_trace(TRACE_INTERNAL, "contacts: delete item %d", get_type_uid (osync_change_get_uid (change)));
			gpesync_client_exec_printf (env->client, "del vcard %d", client_callback_string, &response, NULL, get_type_uid (osync_change_get_uid (change)));
			break;
		case OSYNC_CHANGE_TYPE_ADDED:
		  	osync_data_get_data(osync_change_get_data (change), &data_ptr, &data_size);
			osync_trace(TRACE_INTERNAL, "contacts: adding item: %s", data_ptr);
			gpesync_client_exec_printf (env->client, "add vcard %s", client_callback_string, &response, NULL, data_ptr);
			break;
		case OSYNC_CHANGE_TYPE_MODIFIED:
		  	osync_data_get_data(osync_change_get_data (change), &data_ptr, &data_size);
			osync_trace(TRACE_INTERNAL, "contacts: modifying item %d: %s", get_type_uid (osync_change_get_uid (change)), data_ptr);
			gpesync_client_exec_printf (env->client, "modify vcard %d %s", client_callback_string, &response, NULL, get_type_uid (osync_change_get_uid (change)), data_ptr);
			break;
		default:
	...
	if (parse_value_modified (response, &result, &modified))
	{
		if (!strcasecmp (result, "OK"))
		{
			...
			error = modified;
			osync_trace(TRACE_ERROR, "Couldn't commit contact: %s", error);
...
void gpe_contacts_get_changes(void *userdata, OSyncPluginInfo *info, OSyncContext *ctx)
{
	...
	osync_trace(TRACE_INTERNAL, "Getting uidlists for vcards:");
	...
	if (errmsg)
	{
		...
		{
		  	osync_trace(TRACE_ERROR, "Error getting contact uidlist: %s", errmsg);
			osync_context_report_error (ctx, OSYNC_ERROR_GENERIC, "Error getting contact uidlist: %s", errmsg);
		} else {
	...
	for (iter = uid_list; iter; iter = iter->next)
	{
		...
		osync_trace(TRACE_INTERNAL, "Getting vcard %s", uid);
		gpesync_client_exec_printf (env->client, "get vcard %s", client_callback_gstring, &vcard_data, NULL, uid);
		osync_trace(TRACE_SENSITIVE, "vcard output:\n%s", vcard_data->str);

		report_change (sinkenv, ctx, "contact", uid, modified, vcard_data->str);
 ...
 */
osync_bool gpe_contacts_setup(sink_environment *sinkenv, OSyncObjTypeSinkFunctions functions, gpe_environment *env, OSyncPluginInfo *info, OSyncError **error)
{
